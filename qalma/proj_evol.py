...
def project_operator(
    ...
    """Build the projection of `operator` over the space generated by the
    orthonormal  `basis` of hermitician operators regarding the scalar product
    `sp`.
    """
...
class ProjectedEvolver:
    ...
    def __init__(self, op_basis: dict, sp: Callable, K0: _Qobj = None, deep: int = 0):
        ...
        Used to build hierarchical basis
    ...
    def build_H_tensor(self, K0=None, deep=0):
        """Build the matrix that evolves the orthogonal components of K(t), as
        well as the orthogonal basis to expand it.
        """
        sp = self.sp
        H = self.op_basis["H"]
        Id = self.op_basis["Id"]
    ...
    def evol_k_orth_components(self, k0, ts):
        """Compute `phi_t`, a list with the components of K(t), regarding
        self.orth_basis, for each `t` in `ts`, provided K(0)=K0.
        """
