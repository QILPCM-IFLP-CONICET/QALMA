...
def _(x_op: ProductOperator, y_op: ProductOperator):
    ...
    for site, op_local in y_op.site_factors.items():
...
def _(x_op: ProductOperator, y_value: complex) -> Operator:
    if y_value:
        return ProductOperator(x_op.site_factors, x_op.prefactor * y_value, x_op.system)
...
def _(x_op: ProductOperator, y_op: ScalarOperator):
    ...
    if prefactor:
        return ProductOperator(
            x_op.site_factors, x_op.prefactor * prefactor, x_op.system
        )
    return ScalarOperator(0, x_op.system)
...
def _(x_op: ProductOperator, y_op: ProductOperator):
    ...
    if len(x_op.site_factors) > 1 or len(y_op.site_factors) > 1:
