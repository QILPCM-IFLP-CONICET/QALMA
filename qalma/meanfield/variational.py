...
def compute_t_score(
    ...
    r"""
    Compute the T-score of a variational mean-field state.
...
def compute_variance(
    ...
    compute_t_score : T-score, which normalises this variance by
        :math:`N\,\langle\hat{F}\rangle_\sigma^2`.
    ...
    f_hat = (k - kappa).simplify()
    mean_f, mean_fsq = cast(np.ndarray, sigma.expect([f_hat, f_hat**2]))
    return float(np.real(mean_fsq - mean_f**2))
