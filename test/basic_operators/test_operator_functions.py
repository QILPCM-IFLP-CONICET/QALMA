...
def compare_spectrum(spectrum1, spectrum2, tol=QALMA_TOLERANCE):
...
def test_relative_entropy(key_rho, key_sigma):
    ...
    if key_rho == key_sigma:
        assert (
            abs(rho.tr() - 1) < QALMA_TOLERANCE
            and abs(rho_qutip.tr() - 1) < QALMA_TOLERANCE
        )
        check_equality(relative_entropy(rho, rho), 0, QALMA_TOLERANCE)
        check_equality(qutip.entropy_relative(rho_qutip, rho_qutip), 0, QALMA_TOLERANCE)
    ...
    if rel_error > RELATIVE_ENTROPY_TOLERANCE:
        ...
        assert (
            False
        ), f" in S({key_rho}|{key_sigma}),  {rel_entr} (qalma) !=   {rel_entr_qutip} (qutip)  relative error: ({rel_error*100}%)"
...
def test_eigenvalues():
    ...
    for s in range(CHAIN_SIZE):
        ...
        assert min_err < QALMA_TOLERANCE, f"closest eigenvalue at {min_err}"

    # Fully mixed operator
    spectrum = sorted(eigenvalues(TEST_CASES_STATES["fully mixed"]))
    assert all(abs(s - 0.5**CHAIN_SIZE) < QALMA_TOLERANCE for s in spectrum)
    ...
    # use the qalma routine
    e0 = min(eigenvalues(HAMILTONIAN, sparse=True, sort="low", eigvals=10))
    assert abs(e0 - e0_qutip) < 10 * QALMA_TOLERANCE
